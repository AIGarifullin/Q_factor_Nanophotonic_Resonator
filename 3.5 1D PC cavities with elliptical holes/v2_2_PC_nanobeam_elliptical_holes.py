...
def main(args):
    ...
    for j in range(N_T+2):
        ff[j] = ff_start - N_tap[j]**2/(N_T + 1)**2*(ff_start - ff_end)    # tapering profile of filling fraction
    ...
    for j in range(N_M):
        ...
        y = major_radius*np.sin(np.radians(t)) # is major axis of ellipse
        x = - x_0 + minor_radius*np.cos(np.radians(t)) # is minor axis of ellipse
        
        vertices = []
        for i in range(len(t)):
            vert = mp.Vector3(x[i], y[i], -h/2)
