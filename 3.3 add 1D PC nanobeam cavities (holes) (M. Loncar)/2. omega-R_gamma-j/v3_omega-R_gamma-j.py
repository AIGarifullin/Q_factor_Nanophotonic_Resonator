...
gamma = np.zeros(N, complex)

f = np.genfromtxt("modes_FF_30_k_05.dat", delimiter=",")

# The dependence of gamma(j)
for j in range(N):
#     omega[j] = f[j,1]*g*c
    omega_r = 2*np.pi*Frequency_res 
    R = np.sqrt(ff[j]*a*w/np.pi)
...
#     gamma[j] = (kappa_1[j]**2/(4*kappa_0[j]**2) - delta[j]**2 + 0j)**0.5
    delta[j] = 1 - omega_r*a/(np.sqrt(kappa_0[j])*np.pi*c)
    gamma[j] = (kappa_1[j]**2/(4*kappa_0[j]**2) - delta[j]**2 + 0j)**0.5
