...
def main(args):
    ...
    a = 0.600        # units of um -- lattice constant
    h = 0.220        # units of um -- height of waveguide
    w = 2.300        # units of um -- width of waveguide
    
    H = 2.5          # units of um -- height of substrate
...
#############################################################################################    
    ...
    sz = 2*(pad + dpml) + h + H   # size of cell in z direction
...
#############################################################################################
    ...
    geometry.append(mp.Block(center=mp.Vector3(0,0,-(h/2 + H/2)), size=mp.Vector3(mp.inf,mp.inf,H), material=SiO2))
    ...
    for j in range(N): # number of mirrors
        geometry.append(mp.Cylinder(center=mp.Vector3(xc_r,0,0), radius=R[j], height=h, material=mp.air))
        xc_r = xc_r + a
    
    # Create tapering profile of holes to the left side of the L = 0
    xc_l = -0.5*a # x-coordinate of first hole
    for j in range(N): # number of mirrors
        geometry.append(mp.Cylinder(center=mp.Vector3(xc_l,0,0), radius=R[j], height=h, material=mp.air))
        xc_l = xc_l - a
    ...
    for j in range(N_add): # number of mirrors
        geometry.append(mp.Cylinder(center=mp.Vector3(xc_r,0,0), radius=Rend, height=h, material=mp.air))
        xc_r = xc_r + a
    ...
    for j in range(N_add): # number of mirrors
        geometry.append(mp.Cylinder(center=mp.Vector3(xc_l,0,0), radius=Rend, height=h, material=mp.air))
        xc_l = xc_l - a
