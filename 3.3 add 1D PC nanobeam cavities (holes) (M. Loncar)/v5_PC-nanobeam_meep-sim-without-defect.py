...
#############################################################################################
    ...
    for j in range(N+N_add): # number of mirrors
        geometry.append(mp.Cylinder(center=mp.Vector3(xc_r,0,0), radius=R, height=h, material=mp.air))
        xc_r = xc_r + a
    
    # Create the profile of equal holes to the left side of the L = 0
    xc_l = -0.5*a # x-coordinate of first hole
    for j in range(N+N_add): # number of mirrors
        geometry.append(mp.Cylinder(center=mp.Vector3(xc_l,0,0), radius=R, height=h, material=mp.air))
        xc_l = xc_l - a
